...
boundingbox_map=qmap(location="Chicago",source="google",maptype="terrain")
...
```{r}
MosquitoCountsBySpecies_Date=aggregate(cbind(NumMosquitos,WnvPresent)~Species+Date,data=traindata,sum)
...
```{r}
TempDiff=as.numeric(subset(weatherdata,Station==1)[,5])-as.numeric(subset(weatherdata,Station==2)[,5])
qplot(TempDiff,binwidth=1)
```
